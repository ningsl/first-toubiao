...
$(document).ready(function() {
	...
	var projectPhotoUploader = WebUploader.create({
	    ...
	    // 文件接收服务端。
	    server:contextPath+"/photoController/uploadFile",
	...
	$("#detail-edit-save").on("click",function(event){
		...
		var index;
		$.ajax( {  
		    url:contextPath+"/projectController/updateDetailPart",// 跳转到 action  
		    data:postData,  
		    type:'post',  
		    cache:false,  
		    dataType:'json',  
	...
	$("#contract-pic").on("click",".btn-project-photo-delete",function(){
		...
		var index=layer.confirm('确定删除该照片吗？', {
		    btn: ['确定','取消'] //按钮
					...
					$.ajax( {  
					    url:contextPath+"/photoController/deletePhoto?id="+id,// 跳转到 action  
					    data:id,  
					    type:'get',  
					    cache:false,  
					    dataType:'json',  
