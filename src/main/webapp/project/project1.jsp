...
function handleFieldArray(webField,obj){
	...
	$(function() {
		...
		/* 	sortName : 'name',
			...
			columns : [ [ {
				field : '[]designerSet.name_deigner.name',
				title : '设计师对象',
				width : 75,
				sortable : false,
				hidden : false,
				formatter: function(value,row,index){
					console.log(row);
					var property="designerSet";
					console.log($(this)[$(this).length-1]);
					
					var field=$(this)[$(this).length-1].field;
					
					console.log(field);
					var flagIndex=field.indexOf('_');
					
					/*field字符串形如:前台信息_后台信息  如下*/
					/* 
					0. name                          前台用name,后台用className.name
					1. name_teacherName               前台用name,后台用className.teacherName
					2. designer.name_designer.name    designer对象的name属性  后 designer.name
					3. []designer.name_designer.name  数组designer对象的每一个name属性  后 designer.name
					4. []name_designer.name            返回name的每一个对象                                                  
					*/
					
					/* 若果filed 形如 name 这类简单的字符串，直接返回value */
					if(flagIndex==-1)
						return value;
					
					var webField=field.substr(0,flagIndex);
					console.log(webField);
					
					var result=handleFieldArray(webField,row);
					console.log(result);
...
var items=[
			...
			{
				title:'华北',
				value:"project.department like '%华北%'" ,
				text:'华北'
			},{
				title:'华南',
