...
<c:if test="${fn:contains(sessionInfo.resourceList, '/projectController/deleteProject')}">
	<script type="text/javascript">
		$.canDelete = true;
		//console.log("can delete");
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/projectController/addProject')}">
	<script type="text/javascript">
		$.canAdd= true;
	</script>
</c:if>
...
function handleFieldArray(webField,obj){
	...
	function deleteFun(id) {
		...
		parent.$.messager.confirm('询问', '您是否要删除当前项目？', function(b) {
			if (b) {
					...
					$.post('${pageContext.request.contextPath}/projectController/deleteProject', {
						id : id
					}, function(result) {
						if (result.success) {
	...
	function downloadAll (type){
        ...
        var excelUrl="${pageContext.request.contextPath}/projectController/downloadExcelAll";
        var docxUrl="${pageContext.request.contextPath}/projectController/downloadDocxAll";
        if(type=='excel'){
	...
	function downloadChecked(type){
		var excelUrl="${pageContext.request.contextPath}/projectController/downloadExcelChecked?ids=";
        var docxUrl="${pageContext.request.contextPath}/projectController/downloadDocxChecked?ids=";
		var rows = dataGrid.datagrid('getChecked');
		var idArray = [];
		if (rows.length > 0) {
			var flag = false;
	...
	function downloadFiltered(type){
		var excelUrl="${pageContext.request.contextPath}/projectController/downloadExcelFiltered";
	    var docxUrl="${pageContext.request.contextPath}/projectController/downloadDocxFiltered";
