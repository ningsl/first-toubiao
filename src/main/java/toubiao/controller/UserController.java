...
public class UserController extends BaseController {
	...
	public Json userLogin(User user, HttpSession session, HttpServletRequest request) {
		...
		Json j = new Json();
		if(user.getName()==null || user.getPwd()==null || user.getName().trim().equals("") || user.getPwd().trim().equals("")){
			j.setSuccess(false);
			j.setMsg("用户名或密码 输入为空！");
			return j;
		}
	...
	public DataGrid loginCombogrid(String q, PageHelper ph) {
		logger.info(q);
		logger.info(ph.getRows());
		DataGrid dataGrid=userService.loginCombogrid(q, ph);
		logger.info(dataGrid.getRows().size());
		return dataGrid;
	}
